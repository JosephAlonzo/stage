$(document).ready(function() {
    ...
    $('#usersList tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#usersList').DataTable({
        ...
        columnDefs: [
            ...
            { "name": "roles", "targets": 1 },
            { "name": "isEnabled", "targets": 2 },
            { "name": "actions", "targets": 3, "class": "text-right", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#usersList').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#usersList .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#usersList').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
