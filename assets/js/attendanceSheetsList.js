$(document).ready(function() {
    $('#attendanceSheetsList tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#attendanceSheetsList').DataTable({
        dom: 'Bfrtip',
        columnDefs: [
            { "name": "planning",  "targets": 0, className: "d-none" },
            { "name": "activity",  "targets": 1 },
            { "name": "startDate",  "targets": 2 },
            { "name": "cycle",   "targets": 3 },
            { "name": "numberSessions",  "targets": 4 },
            { "name": "actions",  "targets": 5, "class": "text-center", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#attendanceSheetsList').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#attendanceSheetsList .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#attendanceSheetsList').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
