$(document).ready(function() {
    $('#social_workers_list tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#social_workers_list').DataTable({
        dom: 'Bfrtip',
        columnDefs: [
            { "name": "lastName",  "targets": 0 },
            { "name": "firstName",   "targets": 1 },
            { "name": "origin",  "targets": 2 },
            { "name": "address",  "targets": 3 },
            { "name": "phoneNumber",  "targets": 4 },
            { "name": "email",  "targets": 5 },
            { "name": "city",  "targets": 6 },
            { "name": "actions",  "targets": 7, "class": "text-right", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#social_workers_list').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#social_workers_list .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#social_workers_list').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
