$(document).ready(function() {
    $('#tenantsList tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#tenantsList').DataTable({
        dom: 'Bfrtip',
        columnDefs: [
            { "name": "cdosName", "targets": 0 },
            { "name": "cdosNumber", "targets": 1 },
            { "name": "siret", "targets": 2 },
            { "name": "codeApe", "targets": 3 },
            { "name": "address", "targets": 4 },
            { "name": "city", "targets": 5 },
            { "name": "phoneNumber", "targets": 6 },
            { "name": "email", "targets": 7 },
            { "name": "siteInternet", "targets": 8 },
            { "name": "actions",  "targets": 9, "class": "text-right", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#tenantsList').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#tenantsList .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#tenantsList').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
