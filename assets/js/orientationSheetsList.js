$(document).ready(function() {
    $('#orientationSheetsList tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#orientationSheetsList').DataTable({
        dom: 'Bfrtip',
        columnDefs: [
            { "name": "startDate",  "targets": 0 },
            { "name": "situation",   "targets": 1 },
            { "name": "axe",  "targets": 2 },
            { "name": "photoAuthorization",  "targets": 3 },
            { "name": "sendingDate",  "targets": 4 },
            { "name": "confirmed",  "targets": 5 },
            { "name": "actions",  "targets": 6, "class": "text-right", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#orientationSheetsList').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#orientationSheetsList .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#orientationSheetsList').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
