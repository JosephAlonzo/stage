$(document).ready(function() {
    $('#holidaysList tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#holidaysList').DataTable({
        dom: 'Bfrtip',
        columnDefs: [
            { "name": "name",  "targets": 0 },
            { "name": "startDate",  "targets": 1 },
            { "name": "endDate",  "targets": 2 },
            { "name": "actions",  "targets": 3, "class": "text-right", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#holidaysList').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#holidaysList .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#holidaysList').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
