$(document).ready(function() {
    $('#structuresList tfoot th').each( function (k, v) {
        var title = $(this).text();
        if (title != "Actions")
        {
    ...
    var table = $('#structuresList').DataTable({
        dom: 'Bfrtip',
        columnDefs: [
            { "name": "name", "targets": 0 },
            ...
            { "name": "actions",  "targets": 5, "class": "text-right", "search": false },
        ],
        processing: true,
        serverSide: true,
        ajax: {
            "url": $('#structuresList').data('pathroute'),
            "type": "POST"
        },
        stateSave: true,
        initComplete: function () 
        {
            ...
            api.buttons().container().appendTo('#structuresList .col-md-6:eq(0)');
        ...
        lengthChange: false,
        language: {
            "url": $('#structuresList').data('pathlanguage')
        },
        buttons: [
            { 
                extend: 'pageLength',
